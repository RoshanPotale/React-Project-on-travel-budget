...
import bgImage from "../images/bg3.webp"
...
const LiveBudget = () => {
  ...
  const [expense, setExpense] = useState({ name: '', cost: '', date:'' });
  ...
  const fetchById = async (id) => {
      try {
        const response = await fetch(`https://travelbudget.onrender.com/api/v1/livebudget/${id}`);
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('https://travelbudget.onrender.com/api/v1/livebudget');
      const data = await response.json();
      setTravelData(data);
    } catch (error) {
      console.error("Error fetching data:", error);
  ...
  const handleUpdateClick = async (id) => {
    try {
      ...
      // Set existing details to the form fields
      setTripDetails({
        tripName: data.tripName,
        arrival: data.arrival,
        destination: data.destination,
        totalBudget: data.totalBudget,
        _id: data._id, // Set the correct ID here
      });
  ...
  const handleAddExpense = () => {
    ...
    {
      if(!(expense.name==='' && expense.cost==='' && expense.date===''))
      {
        setExpenses([...expenses, expense]);
        setExpense({ name: '', cost: '',date:'' });
        toast({
          title: 'Expense Added',
  ...
  const deleteTrip = async(id)=>{
      try {
            ...
            await axios.delete(`https://travelbudget.onrender.com/api/v1/livebudget/${id}`); // Replace with your API endpoint
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (tripDetails._id) {
        // If _id exists, it means it's an update, so make a PUT request
        const response = await fetch(`https://travelbudget.onrender.com/api/v1/livebudget/${tripDetails._id}`, {
          method: 'PUT',
          headers: {
        ...
        if (response.ok) {
          ...
          toast({
            title: 'Form Updated',
            description: 'Budget details updated successfully!',
            status: 'success',
            duration: 3000,
            isClosable: true,
          });
        ...
        // If _id doesn't exist, it's a new record, so make a POST request
        const response = await fetch('https://travelbudget.onrender.com/api/v1/livebudget', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
      ...
      setTripDetails({
        ...
        destination: '',
        totalBudget: '',
        _id: '', // Add this line to reset _id after submission
      });
      setExpenses([]);
  ...
  return (
    ...
    >
      <Box width={['100%','90%','40%']} margin={'auto'} marginTop={5} marginBottom={5}
      boxShadow= {'rgba(6, 24, 44, 0.4) 0px 0px 0px 2px, rgba(6, 24, 44, 0.65) 0px 4px 6px -1px, rgba(255, 255, 255, 0.08) 0px 1px 0px inset'}
      // backgroundColor= {'rgba(255, 255, 255, 0.8)'}
      backgroundColor= {'#f9fcff'}
      backgroundImage= {'linear-gradient(147deg, #f9fcff 0%, #dee4ea 74%)'}
      borderRadius={'30px'} padding={5}
      >
        <Heading>Create Live Budget</Heading>
        <Stack spacing={3} mt={4}>
          ...
          <Box display={'flex'} justifyContent={'flex-end'}>
            <Button onClick={handleAddExpense} transition={'all 0.5s ease-in-out'} bg={'red'} color={'white'} _hover={{bg:'red',color:'white',transform:'scale(1.05)'}}>Add Expense</Button>
          </Box>
          <List mt={4}>
            {expenses.map((expense, index) => (
              <ListItem key={index}>
                {expense.name}: {expense.cost} : {new Date(expense.date).toLocaleDateString()}
              </ListItem>
            ))}
          </List>
      ...
      <Box bg={'rgba(11, 4, 81, 0.81)'} height={'fit-content'} width={['100%','90%','50%']} marginLeft={'auto'} marginTop={'100px'} marginRight={'auto'} borderRadius={'20px'} color={'white'}>
      <TableContainer>
              ...
              travelData.map((el)=>{
                return(
                  <Tr key={el._id} color={'yellow'}>
                    ...
                    <Button
                      onClick={() => handleUpdateClick(el._id)} // <--- Corrected to el._id
                      color={'white'}
                      bg={'none'}
                      transition={'all 0.5s ease-in-out'}
                      _hover={{ bg: 'none', color: 'blue', transform: 'scale(1.05)' }}
                    >
                      Update
                    </Button>
              ...
              <List spacing={3} marginTop={'20px'} marginBottom={'20px'}>
                {
                  expenseData.map((el,i)=>{
                    ...
                    <ListItem key={i}>
                      ...
                      {el.name} : {el.cost} : {new Date(el.expenseDate).toLocaleDateString()}
                    </ListItem>
                    )
                  })
                }
